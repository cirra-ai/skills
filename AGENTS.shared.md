...
Before writing a new helper, utility, wrapper, parser, formatter, mapper, or
any small reusable function, **search the codebase for code that already does
it.** Agents reinvent helpers constantly, and the new copies are often buggy.

**Workflow, before you write the function:**

1. Search for existing utilities: `grep` / semantic search for the behavior
   (function names, error-message strings, distinctive constants, the
   transform you need). Check `lib/`, `utils/`, `helpers/`, `shared/`, and
   the nearest module that already owns this domain.
2. If a shared helper exists that covers the need: **use it**. Do not write
   a parallel copy, even if yours is "simpler" or "just for this call site."
3. If you find **one other occurrence** of the same logic inlined: consider
   extracting a shared helper *if that extraction is genuinely helpful*
   (same contract, same edge cases, a name that will stay correct). Do it
   in this change only when the extra diff is small.
4. If the same logic appears in many places, or extracting it would touch
   unrelated call sites, types, or tests: leave those sites alone. Use
   (or add) the helper at the call site you were asked to change. Mention
   the other occurrences; do not expand the PR into a repo-wide refactor.

**Do not:**

- Invent a new `formatX` / `parseY` / `toZ` because you did not look.
- "Improve" a working shared helper by forking it into a local variant.
- Extract a grand abstraction to justify consolidating many slightly
  different copies. Slightly different is often actually different.

A helper that already exists and is tested beats a new one you wrote in
this session.
...
## ⛔ CI monitoring is silent unless something needs attention

When you are watching CI (GitHub Actions, required checks, deploy previews):
**produce no user-facing message** while jobs are pending or green. Poll
with tools only. A "still running…" / "lint passed…" / "all checks green"
update burns tokens and tells the user nothing they cannot see on the PR.

**Report only when something is red or needs a decision:**

- a check failed, was cancelled, or is stuck
- a required check never started
- a result is ambiguous and you need the user to choose

**Do not report:**

- "CI is still running"
- "all checks passed" / "everything is green"
- a per-job scoreboard while the run is in progress or after a clean pass

Green is the default. Silence means you are watching and nothing is wrong.
Fix a red check; do not narrate a green one. A work-complete summary is
about the change you shipped (diff, PR URL), not a CI victory lap.
...
### Angle-bracket HTML tags get silently eaten — don't put them in a body

Writing a literal `<tag>` in a PR or issue body submitted through the API is
unreliable: many tags are **removed outright**, leaving an empty gap where the
name should be. Backticks and fenced code blocks do **not** protect them. The
body you read back is not the body you sent, and nobody notices until a
reviewer reads a sentence with a hole in it.

Measured on `cirra-ai/website` PRs #223 and #227:

| Sent in the body | Stored as |
| --- | --- |
| `` `<main id="main-content" role="main">` `` | `` `` `` (empty) |
| `` `<img loading="lazy">` `` | `` `` `` (empty) |
| `` `<a>` `` | `` `` `` (empty) |
| `` `<link rel="preload" href="…" as="style">` `` | `` `` `` (empty) |
| `` `<h1>` `` | survived |

It is not about attributes — bare `<a>` was eaten while bare `<h1>` survived.
Which tags survive is not documented anywhere we can point to, and we have not
established whether the stripping happens in GitHub or in the MCP layer. Treat
the whole construct as unreliable rather than trying to predict it. Separately,
`'`, `"`, `&` and `->` come back HTML-escaped (`&#39;`, `&#34;`, `&amp;`,
`-&gt;`) — harmless, but it means a sanitizing pass is running over the body.

**So: never write a bare angle-bracketed tag in a PR body, issue body, or
review comment.** Say it a way that cannot be eaten:

- name the element in prose — "a single `main` landmark", "the `img` tags carry
  no `width`/`height`"
- quote a real line from the diff, which reviewers can read in the file anyway
- in a **committed file** (like this one) the `lt`/`gt` HTML entities render
  correctly, so `&lt;main&gt;` is fine here. In a **PR body** they are not: the
  same sanitizing pass escapes the leading `&`, so the reader sees the escape
  sequence instead of the tag. Inside a body, use prose.

**Verify after writing.** The PR-description re-read this section already
mandates is where you catch this: after any `create_pull_request` or
`update_pull_request`, read the body back and check that nothing you wrote came
back empty. A pair of adjacent backticks with nothing between them is the
signature.
...
PR-description, or evidence rules above. Watching CI is not chat: see
"CI monitoring is silent unless something needs attention".
...
- **Questions**: restate the ask in this turn (see "Re-ask; do not point at
  'the XXX question'").

If you are unsure of the exact path or name, say so explicitly rather than
giving a vague reference.

## ⛔ Re-ask; do not point at "the XXX question"

When you need something from the user (an answer, a secret, a click, a
decision), **ask it again in this message** — short, complete, and
self-contained. Do not refer to it as "the XXX question", "the earlier
prompt", "as above", or "see my previous message" when that ask is buried
in an earlier wall of text.

The user is human. They will not scroll 200 lines to reconstruct what
"the deploy question" was. You have the transcript; they are in the middle
of something else.

**Do:**

- Restate the ask in one or two sentences in this turn.
- If the source is a doc, issue, or PR comment: include the URL and enough
  quoted or searchable text that they can find it without rereading the
  chat.
- Repeat the options and your recommended default if you already had them.

**Do not:**

- "Waiting on the XXX question"
- "Once you answer the earlier question I can proceed"
- A nickname for a question from a previous turn, a questionnaire, or a
  form the user cannot see without scrolling
