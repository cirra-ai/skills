...
Agent Script transforms agent development from prompt-based suggestions to **code-enforced guarantees**. This skill guides you through writing, debugging, testing, and deploying Agentforce agents using the Agent Script DSL and the Cirra AI MCP Server.

This refactored version replaces Salesforce CLI commands with Cirra AI MCP Server tools while preserving all Agent Script syntax, patterns, and production knowledge.
...
### Phase 5: Deployment with Cirra AI MCP
...
- cirra_ai_team: Your team identifier
- scope: Agent management scope
```
...
Call: soql_query
- sObject: User
- whereClause: Profile.Name = 'Einstein Agent User' AND IsActive = true
- Verify default_agent_user exists in target org
```
...
- metadata: Array with agent config, script content, metadata
         OR
Call: metadata_update (for existing agents)
- type: AiAuthoringBundle
- metadata: Updated agent configuration
```
...
- fullNames: [AgentName]
- Verify current agent state before updating
```
...
Tool: soql_query
Pattern: SELECT Username, Id FROM User
         WHERE Profile.Name = 'Einstein Agent User'
         AND IsActive = true

# Retrieve Existing Agent
Tool: metadata_read
Pattern: Read AiAuthoringBundle or GenAiPlannerBundle by name

# Deploy New Agent
Tool: metadata_create
Pattern: Create AiAuthoringBundle metadata with agent content

# Update Existing Agent
Tool: metadata_update
Pattern: Update AiAuthoringBundle with new agent script

# Query Action Definitions
Tool: soql_query
Pattern: Query GenAiFunction or similar to verify actions exist
```
