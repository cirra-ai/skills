...
Run a complete Salesforce org audit across Apex, Flows, and LWC. Follow these phases in order.
...
3. Write each class body to `./audit_output/intermediate/apex/<ClassName>.cls` before scoring —
   this preserves the source for later HTML report linking
4. Score using the 150-point rubric from the `cirra-ai-sf-apex` skill (preloaded in your context)
...
2. Fetch Flow XML via `metadata_read` in batches — start with 25 flows per call. If the response
   is too large or an error is thrown, halve the batch size and retry. Continue until all XMLs
   are collected.
3. Write each flow XML to `./audit_output/intermediate/flows/<DeveloperName>.flow-meta.xml`
   before scoring
4. Score using the 110-point rubric from the `cirra-ai-sf-flow` skill (preloaded in your context)
...
4. Score using the rubric from the `cirra-ai-sf-lwc` skill (preloaded in your context)
