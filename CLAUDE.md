...
3. **Never write a bare `Object__c` / `Field__c` / `__r` / `__mdt` in Markdown prose or tables.** A pair of `__` is Markdown strong-emphasis, so `prettier --write` (which every push runs) silently rewrites `Amount__c to Invoice__c` into `Amount**c to Invoice**c` — a wrong API name, rendered as bold. This has shipped broken API names in published docs more than once. Prefer backticks (`` `Invoice__c` `` — emphasis doesn't apply inside a code span, so prettier leaves it alone); in plain text, escape both pairs (`Invoice\_\_c`). Re-run `npx prettier --write` after editing and confirm nothing turned into `**`. **This one is enforced:** `scripts/validate-skills.sh` fails on a corrupted name anywhere under `skills/`, so CI catches it. Check a tree yourself with `python3 scripts/check_md_api_names.py` (defaults to `skills/`; pass `.` for the whole repo). The checker reads prose only — matches inside code spans and fenced blocks are ignored, since emphasis doesn't apply there — and honors an `md-api-names: allow` HTML comment on the offending line or the one above.
4. **Every MCP tool call in a skill must match the Cirra AI MCP Server schema.** `soql_query` and `tooling_api_query` take `sObject`, `fields` and `whereClause` (all required; `orderBy`/`limit`/`groupBy` are separate parameters and there is no `query=` string); `sobject_dml` delete takes `recordIds`, not `records`; there is no `orgAlias` parameter. The canonical signatures live in `shared/references/cirra-mcp-tools.md` — link it from the skill rather than re-describing tools. **Enforced:** `scripts/validate-skills.sh` runs `scripts/check_mcp_signatures.py` on every skill (fenced blocks and inline code only); an example that shows a wrong call on purpose can carry an `mcp-signatures: allow` HTML comment on the line above.
