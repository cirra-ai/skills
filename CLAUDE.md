...
- `dist/agent-plugins/` — Generated Agent Plugins 1.0 package (`scripts/package-agent-plugins.sh`). **Beta** for non-OpenAI clients. Extension namespace `ai.cirra`. Validated against vendored schemas in `scripts/schemas/agent-plugins/1.0.0/`. Published separately from the Claude and OpenAI zips (AGENT-3).
