...
3. **Never write a bare `Object__c` / `Field__c` / `__r` / `__mdt` in Markdown prose or tables.** A pair of `__` is Markdown strong-emphasis, so `prettier --write` (which every push runs) silently rewrites `Amount__c to Invoice__c` into `Amount**c to Invoice**c` — a wrong API name, rendered as bold. This has shipped broken API names in published docs more than once. Prefer backticks (`` `Invoice__c` `` — emphasis doesn't apply inside a code span, so prettier leaves it alone); in plain text, escape both pairs (`Invoice\_\_c`). Re-run `npx prettier --write` after editing and confirm nothing turned into `**`. Repo-wide sweep: `git ls-files -z | xargs -0 grep -nE '\*\*(c|r|mdt|x|e|b|kav|Share|History|Feed|Tag|ChangeEvent)\b'` — expect hits only in the generated `plugins/` mirror, deliberate acronym bolding (`**S**ingle Responsibility`) in `skills/sf-apex/`, and the counter-example quoted above.
