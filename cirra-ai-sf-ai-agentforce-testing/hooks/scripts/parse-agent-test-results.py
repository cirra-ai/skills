...
def parse_text_results(output: str) -> dict:
    ...
    return {"summary": summary, "failures": failures, "topic_coverage": [], "action_coverage": []}
...
def categorize_failure(failure: dict) -> dict:
    ...
    if category == "topic_routing" or "topic" in error_msg.lower():
        ...
        if actual:
            analysis["root_cause"] = (
                f"Wrong topic selected: expected '{expected}' but got '{actual}'"
            )
            analysis["suggested_fix"] = (
                "Improve topic scope descriptions to better match the utterance intent"
            )
        else:
            analysis["root_cause"] = "No topic matched for utterance"
            analysis["suggested_fix"] = (
                "Add topic scope examples or adjust classificationDescription"
            )
    ...
    # Action invocation failures
        ...
        if expected_actions and not actual_actions:
            ...
            action_names = [
                a.get("name", str(a)) if isinstance(a, dict) else str(a) for a in expected_actions
            ]
            ...
            analysis["suggested_fix"] = (
                "Review action descriptions and topic instructions for clarity"
            )
        else:
    ...
    # Guardrail failures
        ...
        if expected_behavior == "guardrail_triggered":
            ...
            analysis["suggested_fix"] = (
                "Add fallback topic or improve system instructions for off-topic handling"
            )
        else:
    ...
    # Escalation failures
        ...
        if expected_behavior == "escalation_triggered":
            ...
            analysis["suggested_fix"] = (
                "Add escalation action or improve escalation trigger instructions"
            )
...
def format_output(results: dict) -> str:
    ...
    if failures:
        ...
        for failure_type, items in categorized.items():
            ...
            for i, (failure, analysis) in enumerate(items, 1):
                ...
                if failure.get("utterance"):
                    utterance = (
                        failure["utterance"][:80] + "..."
                        if len(failure.get("utterance", "")) > 80
                        else failure.get("utterance", "")
                    )
                ...
                if failure.get("error_message"):
                    msg = (
                        ...
                        if len(failure.get("error_message", "")) > 100
                        else failure.get("error_message", "")
                    )
        ...
        if any(
            ft in categorized
            for ft in ["ACTION_NOT_INVOKED", "WRONG_ACTION_SELECTED", "ACTION_INVOCATION_FAILED"]
        ):
        ...
        lines.append(
            f"   sf agent test run --api-name {summary['agent_name']}_Tests --wait 10 --target-org [alias]"
        )
        lines.append("")
