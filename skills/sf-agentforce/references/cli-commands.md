...
| `--json`        | No       | Not supported — outputs nothing. Omit this flag. |
...
| `--name`             | Yes (with `--json`) | Name for the authoring bundle                             |
| `--no-spec`          | No                  | Generate without a spec file                              |
| `--force-overwrite`  | No                  | Skip duplicate-detection prompt and overwrite (v2.125.1+) |
...
> **v2.126.4 change**: When `--json` is specified, `--name` is now **required**. Previously the CLI would still prompt interactively even with `--json`, which was incorrect for scripting/CI contexts.
