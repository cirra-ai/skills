...
class MetadataValidator:
    ...
    def _add_issue(self, category: str, severity: str, message: str, deduction: int = 0):
        ...
        if category in self.categories:
            self.categories[category]["issues"].append({"severity": severity, "message": message})
            if deduction > 0:
    ...
    def _validate_structure(self):
        ...
        if not root_tag.startswith("{http://soap.sforce.com/2006/04/metadata}"):
            ...
            if not root_tag.startswith("{"):
                self._add_issue(
                    "structure_format", "WARNING", "Missing Salesforce metadata namespace", 5
                )
        ...
        if "/objects/" in self.file_path:
            if self.metadata_type == "CustomField" and "/fields/" not in self.file_path:
                self._add_issue(
                    "structure_format",
                    "WARNING",
                    "Field files should be in objects/[ObjectName]/fields/",
                    3,
                )
            if self.metadata_type == "ValidationRule" and "/validationRules/" not in self.file_path:
                self._add_issue(
                    "structure_format",
                    "WARNING",
                    "Validation rules should be in objects/[ObjectName]/validationRules/",
                    3,
                )
    ...
    def _validate_naming(self):
        ...
        if self.metadata_type in ["CustomObject", "CustomField"]:
            ...
            base_name = self.file_name.replace(".object-meta.xml", "").replace(
                ".field-meta.xml", ""
            )
            if not base_name.endswith("__c") and not base_name.startswith("standard-"):
                self._add_issue(
                    "naming_conventions",
                    "WARNING",
                    f"Custom metadata should have __c suffix: {base_name}",
                    3,
                )
            ...
            if api_name and not re.match(r"^[A-Z][a-zA-Z0-9_]*$", api_name.replace("__c", "")):
                self._add_issue(
                    "naming_conventions", "INFO", f"API name should use PascalCase: {api_name}", 2
                )
        ...
        if label and api_name:
            ...
            if label.lower() == api_clean.lower():
                self._add_issue(
                    "naming_conventions",
                    "INFO",
                    "Label should be user-friendly, not just API name",
                    1,
                )
            ...
            for pattern, suggestion in abbreviation_patterns:
                if re.search(pattern, label, re.IGNORECASE):
                    self._add_issue(
                        "naming_conventions",
                        "INFO",
                        f'Avoid abbreviations in labels. Consider using "{suggestion}"',
                        1,
                    )
                    break
    ...
    def _validate_data_integrity(self):
        ...
        if self.metadata_type == "CustomField":
            ...
            if required == "true" and not default_value:
                if field_type not in ["Lookup", "MasterDetail"]:
                    self._add_issue(
                        "data_integrity",
                        "INFO",
                        "Required fields should consider having a default value",
                        2,
                    )
            ...
            if field_type in ["Number", "Currency", "Percent"]:
                ...
                if not precision or not scale:
                    self._add_issue(
                        "data_integrity",
                        "WARNING",
                        f"{field_type} fields should specify precision and scale",
                        3,
                    )
            ...
            if field_type in ["Lookup", "MasterDetail"]:
                ...
                if not rel_name:
                    self._add_issue(
                        "data_integrity",
                        "WARNING",
                        "Relationship fields should have relationshipName",
                        3,
                    )
                ...
                if field_type == "Lookup":
                    ...
                    if not delete_constraint:
                        self._add_issue(
                            "data_integrity",
                            "INFO",
                            "Consider setting deleteConstraint for lookup fields",
                            2,
                        )
            ...
            if not error_message:
                self._add_issue(
                    "data_integrity", "WARNING", "Validation rules must have an error message", 5
                )
            elif len(error_message) < 10:
                self._add_issue("data_integrity", "INFO", "Error messages should be descriptive", 2)
        ...
        for pattern in self.SENSITIVE_PATTERNS:
            if re.search(pattern, combined_text, re.IGNORECASE):
                self._add_issue(
                    "security_fls",
                    "CRITICAL",
                    "Potential sensitive data field detected. Ensure proper FLS and encryption.",
                    10,
                )
                break
        ...
        if self.metadata_type == "CustomObject":
            ...
            if sharing_model == "ReadWrite":
                self._add_issue(
                    "security_fls",
                    "INFO",
                    "Public Read/Write sharing model - verify this is intentional",
                    2,
                )
            ...
            for perm in user_perms:
                ...
                if enabled == "true" and name in ["ModifyAllData", "ViewAllData"]:
                    self._add_issue(
                        "security_fls",
                        "WARNING",
                        f"{name} permission enabled - use with caution",
                        5,
                    )
    ...
    def _validate_documentation(self):
        ...
        if self.metadata_type in ["CustomObject", "CustomField", "PermissionSet"]:
            if not description:
                self._add_issue(
                    "documentation", "WARNING", "Add a description to explain the purpose", 5
                )
            elif len(description) < 20:
                self._add_issue("documentation", "INFO", "Description should be more detailed", 2)
        ...
        if self.metadata_type == "CustomField":
            ...
            if field_type not in ["Formula", "Summary"] and not help_text:
                self._add_issue(
                    "documentation", "INFO", "Consider adding help text for user guidance", 2
                )

        # Validation rules should have good error messages
        if self.metadata_type == "ValidationRule":
            error_message = self._get_text(self.root, "errorMessage")
            if error_message and not any(
                word in error_message.lower() for word in ["please", "must", "should", "required"]
            ):
                self._add_issue(
                    "documentation",
                    "INFO",
    ...
    def _validate_best_practices(self):
        ...
        if self.metadata_type == "Profile":
            self._add_issue(
                "best_practices",
                "INFO",
                "Consider using Permission Sets instead of Profiles for granular access",
                3,
            )
            ...
            if field_type == "Formula":
                ...
                if formula and re.search(r'["\'][0-9a-zA-Z]{15,18}["\']', formula):
                    self._add_issue(
                        "best_practices",
                        "WARNING",
                        "Avoid hardcoded IDs in formulas - use Custom Settings or Custom Metadata",
                        5,
                    )
                ...
                if value_set is not None:
                    ...
                    if len(values) > 10:
                        self._add_issue(
                            "best_practices",
                            "INFO",
                            "Consider using a Global Value Set for reusable picklist values",
                            2,
                        )
        ...
        if self.metadata_type == "ValidationRule":
            ...
            if formula and "$Permission" not in formula and "$Setup" not in formula:
                self._add_issue(
                    "best_practices",
                    "INFO",
                    "Consider adding a bypass mechanism for admin/integration users",
                    3,
                )
...
def main():
    ...
    print("\nCategory Breakdown:")
    for _cat_key, cat_data in results["categories"].items():
        score = cat_data["score"]
